...
Multiplexer.prototype._transform = function (connection, encoding, done) {
  ...
  if (connection['@context'] || connection['departureTime'].getTime() < this._departureTime.getTime()) {
    done();
  } else if (!this._merge) {
...
Multiplexer.prototype._process = function (connection, connectionInQueue, done) {
  ...
  if (connectionInQueue['departureTime'].getTime() <= connection['departureTime'].getTime()) {
    ...
    // 2. Set the bar higher if new connection is higher then current synced departureTime
    if (connection['departureTime'].getTime() > this._departureTime.getTime()) {
...
Multiplexer.prototype._removePreviousConnections = function (connection, connectionInQueue, done) {
  var self = this;
  if (connectionInQueue['departureTime'].getTime() < this._departureTime.getTime()) {
...
Multiplexer.prototype.startMergingAgain = function (departureTimeSync) {
  ...
  if (this._firstConnectionInQueue && this._firstConnectionInQueue['departureTime'].getTime() < this._departureTime.getTime()) {
